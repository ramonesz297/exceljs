...
class BaseXform {
  ...
  /**
   * @param {Object} model
   * @param {{useWorker: boolean} | undefined} options
   * @returns {Promise<ArrayBuffer>} model
   */
  toArrayBuffer(model, options) {
    const xmlStream = new XmlStream(options);
    this.render(xmlStream, model);
    return xmlStream.toArrayBuffer();
  }
