...
class XLSX {
  ...
  async addSharedStrings(zip, model) {
    if (model.sharedStrings && model.sharedStrings.count) {
      zip.append(model.sharedStrings.toArrayBuffer(model.sharedStrings.model, {useWorker: true}), {
  ...
  async addWorksheets(zip, model) {
    ...
    // write sheets
    model.worksheets.forEach(worksheet => {
      let xmlStream = new XmlStream({useWorker: true});
      worksheetXform.render(xmlStream, worksheet);
      zip.append(xmlStream.toArrayBuffer(), {name: `xl/worksheets/sheet${worksheet.id}.xml`});
